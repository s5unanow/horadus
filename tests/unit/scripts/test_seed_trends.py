...
class _FakeSession:
    def __init__(self, existing_trends: list[Trend | None]) -> None:
        self._existing_trends = list(existing_trends)
        self.added: list[Trend] = []
        self.commit_calls = 0

    async def execute(self, statement: object) -> _ScalarResult:
        del statement
        return _ScalarResult(self._existing_trends.pop(0))
...
@pytest.mark.asyncio
async def test_seed_trends_updates_runtime_trend_id_on_existing_trends(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    seed_trends_module = _load_seed_trends_module()
...
""",
    ...
    assert existing.runtime_trend_id == "signal-watch-v2"
    assert existing.definition["id"] == "signal-watch-v2"
