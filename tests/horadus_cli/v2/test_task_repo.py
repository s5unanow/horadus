...
def test_task_record_prefers_structured_spec_reference(
    ...
    resolution = task_repo_module.task_spec_resolution_for_task(
        "TASK-381",
        record.raw_block if record is not None else "",
    )

    assert record is not None
    assert record.spec_paths == ["tasks/specs/381-canonical.md"]
    assert resolution["selector"] == "backlog-spec-reference"
    assert resolution["selected_paths"] == ["tasks/specs/381-canonical.md"]


def test_task_spec_resolution_uses_supersession_metadata(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    resolution = task_repo_module.task_spec_resolution_for_task("TASK-381")

    assert resolution["ambiguous"] is False
    assert resolution["selected_paths"] == ["tasks/specs/381-new.md"]
    candidates = {item["path"]: item for item in resolution["candidates"]}
...
def test_task_spec_resolution_covers_ambiguous_active_metadata(
    tmp_path: Path,
) -> None:
    specs_dir = tmp_path / "tasks" / "specs"
    specs_dir.mkdir(parents=True)
...
def test_task_spec_resolution_covers_reference_and_front_matter_edges(
    tmp_path: Path,
) -> None:
    specs_dir = tmp_path / "tasks" / "specs"
    specs_dir.mkdir(parents=True)
    ...
    assert spec_resolution_module.spec_reference_paths_from_text(
        "**Spec**: , 381-shorthand.md, ignored.txt, 381-shorthand.md"
    ) == ["tasks/specs/381-shorthand.md"]
    ...
    unterminated = spec_resolution_module.resolve_task_spec_paths(
        repo_root=tmp_path,
        task_id="TASK-381",
        raw_block="**Spec**: `381-unterminated.md`",
    )
    malformed_top = spec_resolution_module.resolve_task_spec_paths(
        repo_root=tmp_path,
        task_id="TASK-381",
        raw_block="**Spec**: `381-malformed-top.md`",
    )
    assert unterminated.to_payload()["candidates"][0]["retrieval_ready"] is False
    assert malformed_top.to_payload()["candidates"][0]["retrieval_ready"] is False
