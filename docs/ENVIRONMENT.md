...
| `API_KEYS_PERSIST_PATH` | empty | Optional writable file path for persisted runtime API key metadata. Keep it separate from read-only `*_FILE` secret mounts, include an explicit parent directory, and use a private parent directory (`0700`) plus owner-only file mode (`0600`). |
