...
_PLANNING_CANONICAL_EXAMPLE_PATH = "tasks/specs/275-finish-review-gate-timeout.md"
...
def _changed_planning_artifact_paths(repo_root: Path) -> tuple[str, ...]:
    ...
    try:
        merge_base_result = subprocess.run(  # nosec B603
            [git_bin, "merge-base", "HEAD", _PLANNING_CHANGED_DEFAULT_BASE_REF],
            cwd=repo_root,
            check=False,
            capture_output=True,
            text=True,
...
def _validate_planning_artifact(
    *,
    ...
    if relative_path == "tasks/specs/TEMPLATE.md":
        ...
        if _PLANNING_SPEC_SECTION_HEADING not in content:
            issues.append(
                DocsFreshnessIssue(
                    level="warning",
    ...
    if relative_path == "tasks/exec_plans/TEMPLATE.md":
        ...
        if _PLANNING_EXEC_PLAN_SECTION_HEADING not in content:
            issues.append(
                DocsFreshnessIssue(
                    level="warning",
    ...
    if relative_path == "tasks/BACKLOG.md":
        for backlog_task_id in sorted(_extract_task_ids(backlog_text)):
            planning_state = _planning_state_for_task(
                repo_root,
                task_id=backlog_task_id,
                backlog_text=backlog_text,
            )
            if planning_state["state"] != "applicable_backlog_only_missing_artifact":
                continue
            issues.append(
                DocsFreshnessIssue(
                    level="warning",
    ...
    if relative_path.startswith("tasks/specs/"):
        ...
        if _PLANNING_SPEC_SECTION_HEADING not in content:
            issues.append(
                DocsFreshnessIssue(
                    level="warning",
                    rule_id="planning_spec_section_missing",
                    message=f"{relative_path} should include {_PLANNING_SPEC_SECTION_HEADING}.",
                    path=relative_path,
                )
            )
            return tuple(issues)
        for gate_label in _PLANNING_CORE_GATE_LABELS:
        ...
        for gate_label in _PLANNING_CONDITIONAL_GATE_LABELS:
            if gate_label not in content:
                issues.append(
                    DocsFreshnessIssue(
                        level="warning",
        ...
        if _PLANNING_EXEC_PLAN_SECTION_HEADING not in content:
            issues.append(
                DocsFreshnessIssue(
                    level="warning",
                    rule_id="planning_gate_outcomes_missing",
                    message=f"{relative_path} should include {_PLANNING_EXEC_PLAN_SECTION_HEADING}.",
                    path=relative_path,
                )
            )
...
def run_docs_freshness_check(
    *,
    ...
    effective_planning_paths = (
        ...
        else _changed_planning_artifact_paths(repo_root)
    )
