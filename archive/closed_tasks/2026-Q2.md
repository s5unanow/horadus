...
Resolve the dependency-audit CVEs surfaced during eval-label cleanup work
without mixing that eval-label scope into this task.

**Files**: `uv.lock`

**Acceptance Criteria**:
- [ ] Update urllib3 to a non-vulnerable version fixed by 2.7.0 or newer while preserving dependency constraints.
- [ ] Update Mako to a non-vulnerable version fixed by 1.3.12 or newer while preserving dependency constraints.
- [ ] Run dependency-audit and the canonical local gate required for dependency changes.
...
Apply the Deep Research minor-correction findings as a scoped eval/taxonomy
cleanup. No baseline_probability changes are in scope.

**Assessment-Ref**:
- /Users/s5una/Downloads/deep-research-report.md

**Files**: `config/trends/ai-human-control-expansion.yaml`, `config/trends/south-america-agri-supply-shift.yaml`, `ai/eval/gold_set.jsonl`

**Acceptance Criteria**:
- [x] Rename ai-control signal llm_gatekeeping_deployment to ai_gatekeeping_deployment in config and affected gold-set labels.
- [x] Rename south-america-agri-supply-shift signal trade_agreement_expansion to market_access_or_trade_barrier_easing in config and affected gold-set labels.
- [x] Patch eval-0094 and eval-0095 Tier-1 scores to remove residual three-seas-bloc-consolidation false positives per the report.
- [x] Run taxonomy validation, eval audit --fail-on-warnings, and targeted eval tests relevant to gold-set/taxonomy changes; document any benchmark baseline supersession decision.

**Completion Notes**:
- 2026-05-12: Strict taxonomy validation was attempted and still fails on the repo's pre-existing sparse Tier-1 exact-key requirement; transitional taxonomy validation passed for this scoped taxonomy/gold-set change.
- 2026-05-12: Gold-set labels changed, so existing benchmark baseline comparisons are superseded for pass/fail gating. No new `ai/eval/baselines/current.json` is promoted in this task because no prompt/model promotion decision is in scope; run a fresh benchmark before any future promotion decision against the updated dataset.
...
Resolve the dependency-audit blocker surfaced by TASK-413 by upgrading pip from
26.1 to the fixed 26.1.2 lockfile version.

**Assessment-Ref**:
- TASK-413 local-gate dependency-audit

**Files**: `uv.lock`, `pyproject.toml`

**Acceptance Criteria**:
- [ ] Upgrade the locked pip package to 26.1.2 or newer without broad dependency churn.
- [ ] Run dependency audit and the canonical local gate.
...
Fix deterministic trend-impact mapping defects where keyword collisions or mis-
bucketed phrases silently drop or reverse trend deltas.

**Assessment-Ref**:
- trend-catalog baseline review 2026-06-03

**Files**: `config/trends/*.yaml`, `src/processing/trend_impact_mapping.py`, `tests/unit/processing/test_trend_impact_mapping.py`

**Acceptance Criteria**:
- [ ] Remove or relocate the listed wrong-direction and ambiguous indicator keywords without reducing intended trend coverage.
- [ ] Add targeted regression coverage for the affected Tier-2 mapping cases, including at least one no-delta ambiguity prevention case and one corrected direction case.
- [ ] Run taxonomy validation, eval audit with warnings as failures, targeted trend-impact mapping tests, and the canonical local gate.
