...
async def test_merge_event_records_lineage_closes_source_and_queues_target_replay() -> None:
    async with async_session_maker() as session:
        ...
        refreshed_source = await session.get(Event, source_event.id)
        refreshed_target = await session.get(Event, target_event.id)
        refreshed_trend = await session.get(Trend, trend.id)
        ...
        moved_links = list(
            (
                await session.scalars(
                    select(EventItem).where(EventItem.event_id == target_event.id)
                )
            ).all()
        )
        assert {link.item_id for link in moved_links} == {source_item.id, target_item.id}
        ...
        assert lineage_rows[0].source_event_id == source_event.id
        assert lineage_rows[0].target_event_id == target_event.id
        assert result.target_event_id == target_event.id

        replay_rows = list((await session.scalars(select(LLMReplayQueueItem))).all())
        assert len(replay_rows) == 1
        assert replay_rows[0].event_id == target_event.id
