...
def _parse_review_gate_result(result: subprocess.CompletedProcess[str]) -> ReviewGateResult:
    try:
...
def _review_threads(*, pr_url: str, config: FinishConfig) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        if after_cursor is not None:
            ...
            args.extend(["-F", "after="])
        threads_result = _run_command(args)
        if threads_result.returncode != 0:
            raise ValueError(_result_message(threads_result, "Unable to load PR review threads."))
        try:
...
def full_local_gate_steps() -> list[LocalGateStep]:
    ...
    return [
        ...
        LocalGateStep(
            name="ruff-check",
            command=f"{uv_bin} run --no-sync ruff check src/ tools/ tests/",
        ),
        LocalGateStep(
            name="mypy",
            command=f"{uv_bin} run --no-sync mypy src/ tools/horadus/python",
        ),
        LocalGateStep(
        ...
        while True:
            ...
            try:
                ...
                return _task_blocked(
                    ...
                    exit_code=ExitCode.ENVIRONMENT_ERROR,
                    extra_lines=[str(exc), *_output_lines(review_result)],
                )
