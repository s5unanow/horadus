...
- `horadus tasks finish` must always wait a positive review-gate timeout; actionable current-head review feedback blocks completion, while a silent timeout after the full wait window may continue inside the CLI flow only when current-head required checks are still green. If CI is already red, the command must report that blocker instead of waiting out a later merge/timeout path or bypassing to raw `gh pr merge`.
