...
def _evidence_matches(
    ...
    *,
    ...
    return (
        ...
        and _float_matches(evidence.base_weight, desired.factors.base_weight)
        and _float_matches(evidence.direction_multiplier, desired.factors.direction_multiplier)
        and _float_matches(evidence.credibility_score, desired.factors.credibility)
        and _float_matches(evidence.corroboration_factor, desired.factors.corroboration)
        and _float_matches(evidence.novelty_score, desired.factors.novelty)
        and _float_matches(evidence.evidence_age_days, desired.factors.evidence_age_days)
        and _float_matches(
            evidence.temporal_decay_factor,
            desired.factors.temporal_decay_multiplier,
        )
        and _float_matches(evidence.severity_score, desired.factors.severity)
        and _float_matches(evidence.confidence_score, desired.factors.confidence)
        and _float_matches(evidence.delta_log_odds, desired.delta)
