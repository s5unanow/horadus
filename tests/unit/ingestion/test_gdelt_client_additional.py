...
async def test_fetch_articles_and_request_json_cover_error_paths(
    ...
    client_request.safe_fetcher.get = AsyncMock(side_effect=non_retry_exc)
...
async def test_get_or_create_source_and_store_article_cover_update_and_early_returns(
    ...
    assert created.provider_source_key == gdelt_provider_source_key_from_mapping(created.config)
    refresh_mock.assert_not_awaited()

    existing = SimpleNamespace(
        id=uuid4(),
        provider_source_key=gdelt_provider_source_key(
            query=query.query,
            themes=query.themes,
            actors=query.actors,
...
@pytest.mark.asyncio
async def test_get_or_create_source_preserves_state_across_query_rename(
    mock_db_session,
    mock_http_client,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client = GDELTClient(session=mock_db_session, http_client=mock_http_client)
    original = GDELTQueryConfig(
        name="Original Query",
        query="ukraine",
        themes=["MILITARY"],
        actors=["NATO"],
    ...
    updated = await client._get_or_create_source(renamed)
