...
- Tier 2 metrics separate failure classes as `extraction_validity_failures`, `schema_runtime_failures`, and `deterministic_mapper_failures`; prompt or payload changes should explain which bucket they are intended to reduce.
...
- Tier 2 payloads include a compact `trend_signal_catalog` so the model can phrase factual claims with configured signal vocabulary while deterministic code still owns impact mapping.
- Tier 2 prompts may ask for a primary `categories[0]` tag formatted as `<trend_id>:<signal_type>:<direction>`, but that tag is an extraction hint only. Deterministic mapping must still be able to override it when extracted factual claims contain stronger configured signal evidence.
- Trend indicator keyword updates are part of the deterministic mapper contract. Treat them as taxonomy/mapping changes and validate them with Tier-2 benchmark evidence plus the `taxonomy-safety` behavior suite when risk warrants it.
