...
- If `horadus tasks context-pack TASK-XXX --mode implement --format json` surfaces a caller-aware validation pack, run those dependent suites in addition to the baseline gates; shared Python helpers and shared math require explicit full-repo type checking via `make typecheck`.
...
- Worktree-isolated Codex task sessions are specified in `tasks/specs/394-worktree-isolation-codex-sessions.md`. Until a repo-owned worktree-start wrapper exists, `safe-start`, `finish`, and strict lifecycle verification remain the authoritative task lifecycle; Codex App `execution_environment = "worktree"` is not by itself a complete Horadus isolation contract.
