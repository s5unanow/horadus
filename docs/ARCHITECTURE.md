...
- Live trend state keeps a dedicated `last_decayed_at` clock separate from the
  ...
  advances accrued exponential decay under a row lock, then applies the new
  delta at that boundary; ordinary writes therefore cannot erase pending decay
  or make fresh evidence inherit an older decay interval. Scheduled decay uses
  the same clock, and state activation resets it to the new state boundary.
