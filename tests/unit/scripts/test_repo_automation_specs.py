...
def test_horadus_sprint_autopilot_instructions_cover_resume_and_main_sync() -> None:
    ...
    assert (
        "horadus tasks automation-lock lock --automation-id horadus-sprint-autopilot"
        in instructions
    )
    assert '--owner-pid "$PPID"' in instructions
    assert (
        "horadus tasks automation-lock unlock --automation-id horadus-sprint-autopilot"
        in instructions
    )
