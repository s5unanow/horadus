...
def test_init_uses_tier1_api_key_for_primary_client(
    mock_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client_factory = MagicMock(side_effect=lambda **kwargs: kwargs)
    monkeypatch.setattr(tier1_module, "AsyncOpenAI", client_factory)
    monkeypatch.setattr(tier1_module.settings, "LLM_TIER1_API_KEY", "tier1-key")
    monkeypatch.setattr(tier1_module.settings, "OPENAI_API_KEY", "fallback-key")
    ...
    assert classifier.client == {
        "api_key": "tier1-key",  # pragma: allowlist secret
...
def test_build_secondary_client_covers_all_paths(
    ...
    monkeypatch.setattr(tier1_module.settings, "LLM_TIER1_API_KEY", "")
...
def test_secondary_client_falls_back_to_tier1_key(
    mock_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    classifier = _build_classifier(mock_db_session, secondary_model=None)
